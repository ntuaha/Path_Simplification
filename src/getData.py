...
class DOWNLOAD:
	...
	def download(self,start_dt,end_dt):
		self.start_dt = start_dt
		self.end_dt = end_dt
		server_path = "https://www.cathaybk.com.tw/cathaybk/personal_info14_excel.asp"
		client_path = "%s/%s_%s.xls" % (self.folder,self.start_dt,self.end_dt)
		agent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_3) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.65 Safari/537.31'
		data = "SelCur=USD3%%AC%%FC%%A4%%B8&R1=2&BDate=%s%%2F%s%%2F%s&EDate=%s%%2F%s%%2F%s" % (self.start_dt[0:4],self.start_dt[4:6],self.start_dt[6:8],self.end_dt[0:4],self.end_dt[4:6],self.end_dt[6:8])
		print server_path
		print client_path
		print data
